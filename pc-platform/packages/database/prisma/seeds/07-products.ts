...
export async function seedProducts(prisma: PrismaClient) {
  ...
  const products: Array<{
    ...
    stock?: number;
    spec: object;
    powerW?: { idle?: number; typical: number; peak: number };
    dims?: { length?: number; width?: number; height?: number; weightG?: number };
    eav?: Array<{ key: string; value: string; unit?: string; groupKey?: string }>;
  }> = [
    // ── CPUs ─────────────────────────────────────────────────────
    {
      sku: 'AMD-7950X',
  ...
  for (const p of products) {
    ...
    if (p.componentType === ComponentType.CPU && 'socketType' in p.spec) {
      await prisma.cpuSpec.upsert({
        where: { productId: product.id },
        update: {},
        create: { productId: product.id, ...(p.spec as any) },
      });
    ...
    } else if (p.componentType === ComponentType.MOTHERBOARD) {
    ...
    } else if (p.componentType === ComponentType.STORAGE) {
      await prisma.storageSpec.upsert({
        where: { productId: product.id },
        update: {},
        create: { productId: product.id, ...(p.spec as any) },
      });
    } else if (p.componentType === ComponentType.PSU) {
      await prisma.psuSpec.upsert({
        where: { productId: product.id },
        update: {},
        create: { productId: product.id, ...(p.spec as any) },
      });
    } else if (p.componentType === ComponentType.CASE) {
      await prisma.caseSpec.upsert({
        where: { productId: product.id },
        update: {},
        create: { productId: product.id, ...(p.spec as any) },
      });
    } else if (p.componentType === ComponentType.COOLER) {
      await prisma.coolerSpec.upsert({
        where: { productId: product.id },
        update: {},
        create: { productId: product.id, ...(p.spec as any) },
      });
    } else if (p.componentType === ComponentType.MONITOR) {
